...
// const copyCMaps = async (config, log, build) => {
...
export const CopyCMapFiles: Task = {
  description: 'Copy CMap files',
  async run(config, log, build) {
    // await copyCMaps(config, log, build);
    const cMapsSource = 'node_modules/pdfjs-dist/cmaps'; // Adjust the path as needed
    const cMapsDestination = 'build/testtestcmaps/'; // Adjust the destination folder
    // await build.copy(cMapsSource, cMapsDestination);
    await fs.copy(cMapsSource, cMapsDestination);
  },
};
